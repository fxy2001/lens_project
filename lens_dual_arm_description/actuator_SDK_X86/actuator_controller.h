...
public:
    ...
    /**
     * @brief 启用或禁用EtherCAT数据日志记录（记录发送接收的数据）
     * @param enable true: 启用, false: 禁用
     */
    static void enableEthercatDataLogging(bool enable);
    ...
    // Gets motor temperature of actuator
    int getMotorTemp(const std::string& joint_name, double &motor_temp);
    int getMotorTemp(const std::vector<std::string>& joint_names, std::vector<double>& motor_temps);

    // Gets MOS temperature of actuator
    int getMosTemp(const std::string& joint_name, double &mos_temp);
    int getMosTemp(const std::vector<std::string>& joint_names, std::vector<double>& mos_temps);
